...
angular.module('zeppelinWebApp')
  .controller('ParagraphCtrl', function ($scope, $rootScope, $route, $window, $element, $routeParams, $location,
    ...
    var autoRefresh = function () {
      if ("RUNNING" === $scope.paragraph.status || "PENDING" === $scope.paragraph.status) {
        return;
      }
      else {
    ...
    $scope.clearAutoRefresh = function () {
      if (angular.isDefined($scope.stopAutoRef)) {
        $interval.cancel($scope.stopAutoRef);
        $scope.stopAutoRef = undefined;
      }
    ...
    var setTable = function (type, data, refresh) {
      ...
      var renderTable = function () {
        ...
        $scope.data = [];
        colDef = [];
        for (var idx = 0; idx < $scope.paragraph.result.rows.length; idx++) {
          var dRow = $scope.paragraph.result.rows[idx];
          var row = {};
          for (var i = 0; i < $scope.paragraph.result.columnNames.length; i++) {
            var col = $scope.paragraph.result.columnNames[i].name;
            col = col.replace("[0]", "");
            row[col] = dRow[i];
            if (idx == 0) {
